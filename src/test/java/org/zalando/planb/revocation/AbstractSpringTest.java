...
public abstract class AbstractSpringTest {
    ...
    public void setUpMockTokenInfo() throws Exception {
        stubFor(get(urlEqualTo("/tokeninfo")).withHeader(AUTHORIZATION, equalTo(VALID_ACCESS_TOKEN))
                .willReturn(aResponse().withStatus(OK.value()).withHeader(ContentTypeHeader.KEY, APPLICATION_JSON_VALUE)
                        .withBody(TOKENINFO_RESPONSE)));

        stubFor(get(urlEqualTo("/tokeninfo")).withHeader(AUTHORIZATION, equalTo(INVALID_ACCESS_TOKEN))
        ...
        stubFor(get(urlEqualTo("/tokeninfo")).withHeader(AUTHORIZATION, equalTo(INSUFFICIENT_SCOPES_ACCESS_TOKEN))
                .willReturn(aResponse().withStatus(OK.value()).withHeader(ContentTypeHeader.KEY, APPLICATION_JSON_VALUE)
